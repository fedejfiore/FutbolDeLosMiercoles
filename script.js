...
function actualizarClima() {
    ...
    .then(r => r.json()).then(data => {
        ...
        // Texto del clima añadido
        $('#clima-navbar').html(`<span>${pr.weather[0].main === 'Clear' ? '☀️' : '⛅'}</span> <span>${label} — ${Math.round(pr.main.temp)}°C</span> <span style="font-size:0.7rem; opacity:0.8; margin-left:5px;">(${pr.weather[0].description.toUpperCase()})</span>`);
    });
}
    ...
    $('#detalle-partido-dinamico').html(`
        <div class="flip-card-inner" id="flip-card-match">
            <div class="card-front">
                ...
                <div style="display:flex; justify-content:space-around; width:100%; margin-top:25px;">
                    <div><strong style="color:#d4af37">${s1}</strong><ul style="list-style:none; padding:0; margin-top:10px; line-height:1.6;">${e1}</ul></div>
                    <div><strong style="color:#d4af37">${s2}</strong><ul style="list-style:none; padding:0; margin-top:10px; line-height:1.6;">${e2}</ul></div>
                </div>
                ${pzH}
                <div onclick="girarCarta()" style="cursor:pointer; margin-top:30px;">
