...
function actualizarClima() {
    ...
    fetch(url).then(r => r.json()).then(data => {
        ...
        const f = fechaMiercoles.toLocaleDateString('es-AR', { weekday: 'short', day: 'numeric', month: 'numeric' }).toUpperCase();
        $('#clima-navbar').html(`<span class="clima-icon">⛅</span><span class="clima-text">${f} - ${Math.round(pronostico.main.temp)}°C</span>`);
    }).catch(() => $('#clima-navbar').hide());
}
...
function cargarTabla() {
    Papa.parse(URL_GENERAL, {
        ...
        complete: function(res) {
            ...
            $('#tabla-general').DataTable({"order": [[5, "desc"]], "destroy": true, "responsive": true});
        }
    });
}
...
function cargarPartidosYCronicas() {
    Papa.parse(URL_CRONICAS, { download: true, header: true, complete: function(cronRes) {
        ...
        Papa.parse(URL_EQUIPOS, { download: true, header: false, complete: function(res) {
            const data = res.data; const contenedor = $('#contenedor-fechas'); contenedor.empty();
            const cPel = {}, cPech = {}, cPozo = {}; totalPozoGlobal = 0;
            for (let j = 2; j < data[0].length; j += 4) {
                ...
                for (let i = 3; i < data.length; i++) {
                    ...
                    if(pz) { let m = parseFloat(pz.toString().replace(/[^0-9]/g, '')); if(m){ tPozoGlobal += m; pzF += m; r = n; cPozo[n] = (cPozo[n]||0)+m; } }
                }
                contenedor.append(`<div class="mini-fecha-card" onclick="abrirPartido('${f}', '${e1}', '${e2}', \`${dicCronicas[f.trim()] || ''}\`, '${s1}', '${s2}', '${pzF}', '${r}')">${f}</div>`);
            }
            actualizarTop3(cPel, '#top-pelota', '⚽'); actualizarTop3(cPech, '#top-pechera', '🎽'); renderizarPozo(cPozo);
        }});
    }});
}

function abrirPartido(fecha, e1, e2, cron, s1, s2, pozo, responsable) {
    let pzH = (pozo && pozo != "0") ? `<div style="margin:15px 0; color:#d4af37;"><strong>💰 Pozo:</strong> $${pozo} (${responsable})</div>` : "";
        ...
        <div class="flip-card-inner" id="flip-card-match">
            <div class="card-front">
                ...
                <div style="display:flex; justify-content:space-between; gap:20px;">
                    <div style="width:50%"><strong style="color:#d4af37">${s1}</strong><ul class="lista-equipos-modal">${e1}</ul></div>
                    <div style="width:50%"><strong style="color:#d4af37">${s2}</strong><ul class="lista-equipos-modal">${e2}</ul></div>
                </div>
                ${pzH}
                <div onclick="girarCarta()" style="cursor:pointer; margin-top:10px;">
                    <img src="peter.png" class="img-autor-btn" onerror="this.src='https://via.placeholder.com/60'">
                    <p style="font-size:0.75rem; color:gray;">Crónica de Peter 🔄</p>
                </div>
            </div>
            <div class="card-back" onclick="girarCarta()">
                <div style="margin-bottom:15px; color:#00355E;"><strong>CRÓNICA OFICIAL</strong></div>
...
function cargarMatriz() {
    Papa.parse(URL_MATRIZ, { download: true, header: false, complete: function(res) {
        let html = '<div class="table-responsive"><table class="matrix-table">';
        res.data.forEach((row, i) => {
            html += '<tr>';
            row.forEach((cell, j) => {
                let s = ""; let v = parseInt(cell);
                if(i>0 && j>0 && !isNaN(v) && v>0) { s = `style="background:rgba(39,174,96,${Math.min(v/5,1)}); color:white;"`; }
...
function renderizarPozo(conteoPozo) {
    let h = ''; let total = 0;
    for (const j in conteoPozo) { h += `<div class="top-item"><span>👤 ${j}</span> <span>$${conteoPozo[j]}</span></div>`; total += conteoPozo[j]; }
    h += `<div class="top-item" style="border-top:1px solid #ddd; margin-top:5px;"><strong>Total</strong> <strong>$${total}</strong></div>`;
    $('#top-pozo').html(h);
}
